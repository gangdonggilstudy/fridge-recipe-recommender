...
[`delete_user_complete()`](../modules/db_init.py#L273)가 한 사용자의 **모든 흔적**을 외래키 의존 순서대로 지웁니다 — 커스텀 레시피 자식 행 → 좋아요 → 노출 → 기록 → 냉장고 → 선호 → 제한 → users, 그리고 디스크의 학습 모델까지. GDPR 스타일 "잊혀질 권리" 구현입니다.
