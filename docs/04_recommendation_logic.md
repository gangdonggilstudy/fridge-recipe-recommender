...
코드: [`Scorer._apply_blend()`](../modules/scorer.py#L166). 기록 50건이 쌓이면, 이 사용자만의 **로지스틱 회귀(LR)** 가 5요소를 합칩니다.

ML 피처는 6개입니다(상황을 월·계절로 더 쪼갬):
```
x = [재료 0.403, 소모 0.322, 선호 0.703, 상황 0.783, 월적합 1, 계절적합 1]
```
...
```
z    = -1.5 + (0.202 + 0.644 + 0.211 + 0.626 + 0.400 + 0.200) = 0.783
...
2. **기록 저장** — 점수·상황·월/계절 매칭을 history에 스냅샷
3. **학습 트리거** — 기록이 50건+ 이고 조건 맞으면 개인 AI 재학습

'별로에요'를 누르면 정확히 반대로 되돌립니다(−1.5). 이렇게 행동이 쌓이며 **룰 → 블렌더로 자연스럽게 진화**합니다.
