...
### 사용자 취향 (선호 벡터)
온보딩에서 **메인반찬·담백함**을 좋아한다고 표시 → `{메인반찬: 1.0, 담백함: 0.8}`
...
> ℹ️ 이 표는 **공식 설명용 예시**입니다. 실제 `recipes.db`에선 `suitable_weather`가 **대부분(823개 중 약 96%)** 전체 날씨(맑음·비·눈·더위·추위)로 채워져 날씨 항이 거의 1.0이지만, 일부(약 27개)는 특정 날씨만 지정돼 비/눈/더위 시 0.25 패널티가 실제로 작동합니다. `review_keywords`(예: 든든함)는 현재 전 레시피가 비어 있어 동작하지 않습니다 — 데이터가 갖춰지면 활성화됩니다.
...
flowchart LR
    ...
    B --> C["상위 3개 스타일 확인<br/>(예: 찌개·메인반찬·국/탕)"]
    C --> D["그 외 스타일에 +0.2 보너스<br/>(예: 양식 레시피 가산)"]
    D --> E["2차 정렬<br/>다시 내림차순"]
    E --> F["상위 N개 반환"]
```

**예시**: 1차 정렬에서 상위 3개가 모두 찌개면, 4등 양식 레시피에 다양성 보너스(`0.05 × 0.2 = 0.01`)를 줍니다. 큰 값은 아니지만 비슷한 점수일 때 다른 스타일을 한 칸 올려, **찌개만 5개 뜨는 쏠림**을 막습니다.
