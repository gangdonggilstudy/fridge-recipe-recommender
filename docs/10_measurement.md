...
기록을 **시간순으로 정렬해 뒤 20%만 평가 구간**으로 씁니다(앞 80%는 평가에서 제외). 이 평가는 모델을 다시 학습시키지 않고, 그 구간에 **이미 기록된 추천 순위·선택**으로 랭킹 지표만 계산합니다 — 미래 데이터가 과거 평가에 새는 "누수"를 막는 시간순 홀드아웃입니다(세션 경계 일부 분할은 누수 방지를 우선해 허용).
