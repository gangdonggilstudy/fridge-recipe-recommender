...
"얼마나 골랐나"는 **세는 단위**에 따라 두 지표로 갈립니다. 헷갈리기 쉬우니 꼭 구분하세요.
...
```
"추천 **한 번**에 사용자가 뭐라도 골랐나"를 봅니다. 5장을 보여줘도 세션은 1회이므로 **천장이 100%**. 사용자 직관("아무것도 안 고름 vs 뭐라도 고름")에 가장 가까워, 모니터링의 **전체 지표·일별 추이**가 이 값을 씁니다.
...
```
"보여준 카드 중 몇 %가 클릭됐나"(광고 CTR과 같은 방식). **5장 중 1장만 고르면 천장이 20%**(0.2)인 점에 주의 — 이건 버그가 아니라 카드 단위 정의 때문입니다. 스타일별·시간/날씨/월 차원별·레시피별처럼 **카드 단위 비교**에 씁니다.

ℹ️ 참고: 차원별·레시피별 랭킹은 raw CTR을 그대로 정렬하지 않고 **Wilson 신뢰구간 하한**(`_wilson_lower`, [metrics.py:32](../modules/metrics.py#L32))으로 정렬합니다 — 노출 1회·선택 1회(=CTR 100%)처럼 표본이 적은 항목이 상위를 차지하지 않도록 표본 수를 반영하기 위함입니다. 화면엔 raw CTR을 보여주되 정렬 키만 Wilson 하한입니다.

> **노출 vs 선택**: `impressions`는 "보여졌다"(selected=0으로 시작), 클릭하면 selected=1로 갱신. 두 지표 모두 이 테이블이 단일 출처입니다. → [05. 데이터 모델](05_data_model.md)의 impression vs history.
