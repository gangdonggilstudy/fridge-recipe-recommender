...
음성·영수증 입력은 이 파서를 **공유**하고(단일 출처), 파싱 후 확인 다이얼로그 → 냉장고 저장으로 이어집니다([voice_input.py](../ui/voice_input.py), [receipt_input.py](../ui/receipt_input.py)).
...
사용자에겐 **음식 이름만** 보이지만, 각 카드는 내부적으로 (스타일, 맛) 좌표를 가집니다. 4 스타일 × 6 맛 = **24개 카드**로 취향 공간을 고르게 덮습니다([onboarding.py:13](../ui/onboarding.py#L13)):

| | 매운맛 | 담백함 | 단맛 | 짭짤함 | 고소함 | 감칠맛 |
|---|---|---|---|---|---|---|
| **한식** | 김치찌개 | 된장찌개 | 불고기 | 간장닭조림 | 참깨비빔국수 | 사골국 |
| **양식** | 아라비아타 | 샐러드 | 함박스테이크 | 올리브파스타 | 까르보나라 | 버섯크림파스타 |
| **중식** | 마라탕 | 계란볶음밥 | 탕수육 | 굴소스볶음밥 | 깨소금냉채 | 짬뽕 |
| **일식** | 매운라멘 | 초밥 | 데리야키치킨 | 쯔유소바 | 참깨냉모밀 | 샤부샤부 |
...
> **입력**: 정규식 4단계(분리→단위제거→복합어분해→매칭) 후 실패 시에만 LLM 폴백. **콜드스타트**: 음식 카드(4×6 격자)로 자연스럽게 취향을 받아 비대칭(+1.0/−0.5) 벡터화. **위치**: DB→IP→기본값 3단으로 항상 좌표를 구해 날씨 5종으로 분류. 세 단계 모두 **폴백이 있어 어떤 입력·환경에서도 추천이 멈추지 않습니다.**
