...
1. 이 사용자의 과거 기록을 모두 읽음 → `(X, y)`
   ...
   - **약한 미선택**(`recommendation_impressions`의 안 고른 노출): `y`=0, 신뢰도 0.2
     — 아래 ['약한 미선택 신호'](#선택만-하면-ai가-영영-못-배운다--약한-미선택-신호) 참고
2. **최신 기록일수록 더 중요하게** 가중 (`0.95^거리`) — 옛날 취향보다 요즘 취향 우선.
   약한 미선택은 여기에 **신뢰도 0.2를 추가로 곱해** 영향을 낮춤
...
| 기록 ≥ 50건 + 모델 없음 | **첫 학습** |
| 마지막 학습 후 +25건 누적 | **재학습** |
| "예"도 "아니오"도 통틀어 한 종류뿐 | 학습 보류 (예/아니오 구분 불가) — 아래 '약한 미선택'으로 거의 해소 |
...
**문제.** 로지스틱 회귀는 "예(선택=1)"와 "아니오(미선택=0)"가 **둘 다** 있어야 학습됩니다. 그런데 추천 목록에서 음식을 고르면 그건 "예"로 기록되지만, **안 고른 음식은 `history`에 아예 안 남습니다.** 그래서 `👎 별로에요`를 한 번도 안 누른 사용자는 "예"만 잔뜩 쌓여 → 50건을 넘겨도 **AI가 "아니오"를 못 배워 학습 보류**됩니다. (선호 벡터·룰 추천은 계속 개인화되지만, ML 블렌더만 안 켜집니다.)

**해결.** 화면에 **떴는데 안 고른** 추천을 **"약한 아니오"** 로 활용합니다. 신호를 3단계로 나눕니다:

| 신호 | 의미 | 학습 신뢰도 |
|---|---|---|
| 선택 | "이거 고름" | 강한 예 (1.0) |
| 별로에요 | "명시적으로 싫음" | 강한 아니오 (1.0) |
| **노출됐는데 안 고름** | "이번엔 패스" | **약한 아니오 (0.2)** |

> 💡 **왜 '약한'인가?** 안 고른 게 꼭 싫어서가 아닐 수 있습니다 — 그냥 다른 걸 먼저 골랐을 수도 있죠. 그래서 명시 신호(1.0)보다 **훨씬 약하게(0.2)** 반영해, "참고는 하되 과신하지 않게" 합니다.

**어디에 저장하나?** `history`는 *"내가 실제로 고른 기록"* 이라는 의미를 지켜야 하므로 **건드리지 않습니다**(여기에 미선택을 섞으면 "선택 기록" 숫자·콜드스타트 판정이 다 어긋남). 대신 이미 **모든 노출을 기록**하는 `recommendation_impressions` 테이블을 씁니다 — 거기에 학습용 5피처를 함께 저장해, "안 고른(acted=0)" 행을 약한 아니오로 읽습니다.

> 🧾 **비유**: 가게가 "팔린 메뉴"만 적지 말고 "메뉴판에 올렸는데 안 팔린 것"도 **약하게** 세어두면, 손님 취향을 훨씬 빨리 파악합니다.

**0.2는 어떻게 정했나?** [`scripts/tune_weak_weight.py`](../scripts/tune_weak_weight.py) 실험으로 잡았습니다. "룰과 어긋난 가상 사용자 + 현실적 노이즈"에서 가중치를 0.0~1.0으로 바꿔 추천 정확도(hit@1)를 측정한 결과:

- **0** → 무작위보다 나쁨 (반드시 0보다 커야 함)
- **0.05~1.0** → 룰을 크게 상회(통계적으로 비슷한 수준)
- 그중 **현실적 노이즈에서 낮은 값이 유리** + 개념상 미클릭을 과신하면 안 됨 → **0.2 채택**

실서비스 로그가 쌓이면 [`compare_regimes`](../modules/recommend_eval.py)(룰 vs 블렌더)로 재확인·재조정할 수 있는 **합리적 출발점**입니다.

### 가중치를 조정하는 법 (초보자용 따라하기)

조절할 건 **딱 숫자 하나**입니다. "안 고른 추천을 얼마나 신경 쓸지"를 정하는 값이에요.

**1단계 — 파일 열기**

[`modules/ml_model.py`](../modules/ml_model.py) 를 열고 이 줄을 찾으세요:
...
`0.2` 자리만 바꾸면 됩니다. 감 잡는 표:

| 이렇게 바꾸면 | 뜻 | 결과 |
|---|---|---|
| `0.1` 처럼 **낮추면** | 안 고른 걸 덜 신경 씀 | 추천이 더 **얌전**(룰에 가까움) |
| `0.3` 처럼 **높이면** | 안 고른 걸 더 신경 씀 | 더 **공격적**으로 개인화(노이즈 위험↑) |
| `None` 으로 두면 | 이 기능 끔 | 옛날처럼 동작(별로에요 없으면 학습 안 됨) |

> 권장 범위는 **0.1 ~ 0.3**. 너무 크게(예: 1.0) 두면 "그냥 안 누른 것"을 "싫어함"처럼 과신해 추천이 나빠질 수 있어요.

**3단계 — 저장하고 앱 재시작**

저장 후 앱을 다시 켜면, 그 다음 학습부터 새 값이 적용됩니다. 끝!

---

**(선택) 어떤 값이 좋은지 자동으로 보고 싶다면**

```bash
python scripts/tune_weak_weight.py
```

여러 값을 자동으로 시험해 표로 보여주고, 맨 아래 `[결론]` 줄에 추천을 찍어줍니다. (실험 가정을 바꾸고 싶으면 [스크립트](../scripts/tune_weak_weight.py) 맨 위 상수들을 고치면 됩니다 — 주석에 설명 있음.)

> ⚠️ **진짜 정답은 실제 사용자 데이터입니다.** 위 실험은 가상 사용자 기준이라 출발점일 뿐이에요. 서비스가 운영되면 운영자 페이지의 **룰 vs 블렌더 비교**([`compare_regimes`](../modules/recommend_eval.py))로 실제 클릭률을 보고 최종 값을 정하세요.
...
| 예/아니오 한쪽만 있음 | 학습 보류 (약한 미선택이 보통 '아니오'를 공급해 거의 발생 안 함) |
| 기록 부족 | 룰 레짐 유지 |
...
- **세션 전환율** — 추천 한 번이 선택으로 이어진 비율 (헤드라인)
- **NDCG / Recall / Hit-rate** — 추천 순위가 얼마나 정확한가
- (참고: 룰 vs 블렌더 비교 `compare_regimes`는 코드에 있으나 현재 대시보드엔 미연결)
